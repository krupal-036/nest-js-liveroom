...
export class SystemSettingsService {
    constructor(private readonly systemSettingsRepo: SystemSettingsRepository) {}

    async getSettings() {
        try {
            return await this.systemSettingsRepo.getSystemSettings();
        } catch (error) {
            throw new InternalServerErrorException("Failed to load system settings.");
        }
    }
    ...
    async updateSettings(data: UpdateSystemSettingsData) {
        try {
            ...
            throw new InternalServerErrorException("Failed to update system settings.");
        }
    }

    async isLoginEnabled(): Promise<boolean> {
        const settings = await this.systemSettingsRepo.getSystemSettings();
