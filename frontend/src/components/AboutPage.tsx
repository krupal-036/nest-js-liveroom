...
const CAPABILITIES = [
    {
        icon: FiZap,
        title: "Realtime sync",
        desc: "Messages, presence, typing and reactions stream instantly over WebSockets.",
    },
    {
        icon: FiHash,
        title: "Room system",
        desc: "Create, join, leave and delete rooms with realtime room-list updates.",
    },
    {
        icon: FiPaperclip,
        title: "File sharing",
        desc: "Base64-powered attachments with inline previews and a full-screen viewer.",
    },
    {
        icon: FiMic,
        title: "Voice memos",
        desc: "In-browser recording streamed straight into the conversation.",
    },
    {
        icon: FiSmile,
        title: "Reactions & replies",
        desc: "Per-user emoji reactions and quoted reply threading.",
    },
    {
        icon: FiShield,
        title: "Moderation",
        desc: "Role-based admin panel with disable, blacklist and delete controls.",
    },
    {
        icon: FiSliders,
        title: "Admin dashboard",
        desc: "Live user stats, site-wide login/signup toggles and a full registered-users directory.",
    },
];
...
const STACK = [
    {
        name: "React",
        role: "Frontend UI",
        desc: "Component-driven interface with hooks-based state management.",
    },
    {
        name: "TypeScript",
        role: "Language",
        desc: "End-to-end type safety across components, contexts and models.",
    },
    {
        name: "Tailwind CSS v4",
        role: "Design system",
        desc: "Utility-first styling with a custom CSS-first design-token theme.",
    },
    {
        name: "Socket.IO",
        role: "Realtime layer",
        desc: "Persistent WebSocket transport for messages, presence and typing.",
    },
    {
        name: "NestJS",
        role: "Backend",
        desc: "Scalable Node.js server powering auth, users and room gateways.",
    },
    ...
    return (
        <div className="min-h-dvh bg-cream-soft text-stone-900 dark:bg-ink-deep dark:text-stone-100">
            ...
            <main className="mx-auto w-full max-w-5xl px-4 sm:px-6">
                ...
                <section className="relative py-14 text-center sm:py-20">
                    ...
                    <div className="relative animate-rise-in">
                        ...
                        <p className="mx-auto mt-4 max-w-2xl text-base leading-relaxed text-stone-500 sm:text-lg dark:text-stone-400">
                            A lightweight realtime chat workspace focused on speed, clarity and a
                ...
                {/* Overview */}
                <section
                    className="grid gap-10 border-t border-stone-200/60 py-14 lg:grid-cols-2 lg:items-center dark:border-stone-800/60"
                ...
                >
                    <div>
                        ...
                        <p className="mt-4 leading-relaxed text-stone-600 dark:text-stone-400">
                            NestJS Live Room is a realtime chat workspace built around one simple
                            idea: conversations should feel instant, focused and effortless. Create
                            a room, share the name, and everyone inside can exchange messages,
                            files, voice memos and reactions with zero latency.
                        </p>
                        <p className="mt-4 leading-relaxed text-stone-600 dark:text-stone-400">
                            Beneath the minimal interface sits a production-grade architecture — a
                            NestJS backend handling authentication and room orchestration, Socket.IO
                            powering the realtime transport, and a React + Tailwind CSS frontend
                            tuned for every screen size.
                        </p>
                        <ul className="mt-6 space-y-3">
                            {[
                                ...
                                <li
                                    key={item}
                                    className="flex items-start gap-2.5 text-sm font-medium text-stone-700 dark:text-stone-300"
                                >
                                    <FiCheck
                    ...
                    <div className="rounded-2xl border border-stone-200/80 bg-white p-6 shadow-card dark:border-stone-800/80 dark:bg-ink">
                        <div className="mb-4 flex items-center gap-2 text-[11px] font-bold tracking-wider text-stone-400 uppercase dark:text-stone-500">
                            <FiActivity className="h-3.5 w-3.5 text-emerald-500" /> Live
                            architecture
                        </div>
                        <ol className="space-y-2.5 font-mono text-[13px]">
                            <li className="flex items-center gap-3 rounded-xl border border-stone-100 bg-stone-50 px-4 py-3 font-semibold text-stone-700 dark:border-stone-800 dark:bg-stone-900/50 dark:text-stone-200">
                                ...
                                React client · context state
                            </li>
                            <li aria-hidden className="pl-6 text-stone-300 dark:text-stone-600">
                                ↕ WebSocket (Socket.IO)
                            </li>
                            <li className="flex items-center gap-3 rounded-xl border border-stone-100 bg-stone-50 px-4 py-3 font-semibold text-stone-700 dark:border-stone-800 dark:bg-stone-900/50 dark:text-stone-200">
                                <span className="h-2 w-2 shrink-0 rounded-full bg-emerald-500" />{" "}
                                NestJS gateway · rooms & presence
                            </li>
                            <li aria-hidden className="pl-6 text-stone-300 dark:text-stone-600">
                ...
                {/* Capabilities */}
                <section
                    className="border-t border-stone-200/60 py-14 dark:border-stone-800/60"
                ...
                >
                    ...
                    <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        {CAPABILITIES.map(({ icon: Icon, title, desc }) => (
                            <div
                                key={title}
                                className="flex items-start gap-3.5 rounded-2xl border border-stone-200/80 bg-white p-5 transition-colors hover:border-emerald-300/70 dark:border-stone-800/80 dark:bg-ink dark:hover:border-emerald-500/30"
                            >
                ...
                >
                    ...
                    <p className="mt-4 max-w-3xl leading-relaxed text-stone-600 dark:text-stone-400">
                        A dedicated, role-protected dashboard gives administrators full visibility
                        and control over user accounts, workspace access, and moderation — separate
                        from the everyday chat experience.
                    </p>
                    ...
                    <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                        {ADMIN_STATS.map((stat) => (
                            <div
                                key={stat.label}
                                className="rounded-2xl border border-stone-200/80 bg-white p-5 dark:border-stone-800/80 dark:bg-ink"
                            >
                    ...
                    <div className="mt-8 grid gap-10 lg:grid-cols-2 lg:items-start">
                        ...
                        <div className="rounded-2xl border border-stone-200/80 bg-white p-6 shadow-card dark:border-stone-800/80 dark:bg-ink">
                                ...
                                <div className="flex items-center justify-between rounded-xl border border-stone-100 bg-stone-50 px-4 py-3 dark:border-stone-800 dark:bg-stone-900/50">
                                    <div>
                                        ...
                                        <p className="text-[12px] text-stone-500 dark:text-stone-400">
                                            Toggle public login access
                                        </p>
                                    </div>
                                    <span
                                        className="h-6 w-11 rounded-full bg-stone-300 dark:bg-stone-700"
                                        aria-hidden
                                    />
                                </div>
                                <div className="flex items-center justify-between rounded-xl border border-stone-100 bg-stone-50 px-4 py-3 dark:border-stone-800 dark:bg-stone-900/50">
                            ...
                            <p className="mt-4 text-[12px] leading-relaxed text-stone-500 dark:text-stone-400">
                                Protected accounts (such as the primary admin) are shown as
                                read-only in the users table, so moderation actions can never lock
                                the workspace owner out by accident.
                            </p>
                        </div>
                    </div>
                </section>
                ...
                >
                    ...
                    <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        {STACK.map((tech) => (
                            <div
                                key={tech.name}
                                className="rounded-2xl border border-stone-200/80 bg-white p-5 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-card dark:border-stone-800/80 dark:bg-ink"
                            >
                ...
                <section className="border-t border-stone-200/60 py-14 dark:border-stone-800/60">
                    <div className="rounded-3xl border border-stone-200/80 bg-white p-8 text-center shadow-card sm:p-10 dark:border-stone-800/80 dark:bg-ink">
                        ...
                        <p className="mx-auto mt-4 max-w-3xl text-sm leading-relaxed text-stone-600 sm:text-base dark:text-stone-400">
                            This project demonstrates a complete realtime product: cookie-based
                            authentication, role-based access control, WebSocket event
                            orchestration, file streaming with inline previews, an administrator
                            control panel for user and access management, and a fully theme-aware
                            interface — all wrapped in a fast, accessible UI that works beautifully
