...
export const AuthPage = () => {
    ...
    const inputBase =
        "w-full rounded-xl border bg-white py-3 pl-11 pr-11 text-sm text-stone-900 transition-colors placeholder:text-stone-400 focus:outline-none dark:bg-ink-soft dark:text-cream-soft dark:placeholder:text-stone-500";
    const inputOk =
        "border-stone-200 focus:border-emerald-500 dark:border-stone-700 dark:focus:border-emerald-500";
    const inputErr =
        "border-rose-400 focus:border-rose-500 dark:border-rose-500/60 dark:focus:border-rose-500";
    const iconCls =
        "pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-stone-400 dark:text-stone-500";

    const renderError = (field: keyof ValidationErrors) =>
        fieldErrors[field] && touched[field] ? (
            <p
                className="mt-1.5 ml-1 flex items-center gap-1.5 text-xs font-medium text-rose-600 animate-slide-down dark:text-rose-400"
                role="alert"
            >
                ...
                <div className="relative max-w-md">
                    ...
                    <ul className="mt-8 space-y-4">
                        {[
                            {
                                icon: FiZap,
                                text: "Realtime rooms powered by Socket.IO with presence & typing indicators",
                            },
                            {
                                icon: FiHash,
                                text: "Files, voice memos, reactions and threaded replies — out of the box",
                            },
                            {
                                icon: FiShield,
                                text: "Admin-grade moderation controls built right in",
                            },
                        ].map(({ icon: Icon, text }) => (
                            <li
                                key={text}
                        ...
                        <div className="rounded-3xl border border-stone-200/80 bg-white p-6 shadow-card sm:p-8 dark:border-stone-800/80 dark:bg-ink">
                            <div className="mb-6">
                                ...
                                <p className="mt-1.5 text-sm text-stone-500 dark:text-stone-400">
                                    {isLogin
                                        ? "Sign in to rejoin your rooms."
                                        : "Join in seconds — no setup required."}
                                </p>
                            </div>
                            ...
                            {error && (
                                <div
                                    role="alert"
                                    className="mb-5 flex items-start gap-2.5 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700 animate-slide-down dark:border-rose-500/25 dark:bg-rose-500/10 dark:text-rose-300"
                                >
                                ...
                                {!isLogin && (
                                    <div>
                                        <label
                                            ...
                                            className="mb-1.5 ml-1 block text-xs font-semibold tracking-wide text-stone-500 uppercase dark:text-stone-400"
                                        >
                                            Username
                                        </label>
