...
export const Logo: React.FC<LogoProps> = ({ size = "md", showText = true, className = "" }) => {
    ...
    return (
        <div className={`flex min-w-0 items-center gap-2.5 ${className}`}>
            <span
                className={`${s.box} flex shrink-0 items-center justify-center bg-gradient-to-br from-emerald-500 to-emerald-700 text-white shadow-lg shadow-emerald-600/25`}
            >
                <FiMessageSquare className={s.icon} strokeWidth={2.5} />
            </span>
            {showText && (
                <span
                    className={`${s.text} truncate font-bold tracking-tight text-stone-900 dark:text-stone-100`}
                >
