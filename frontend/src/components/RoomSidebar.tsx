...
const card =
    "rounded-2xl border border-stone-200/80 bg-white shadow-card dark:border-stone-800/80 dark:bg-ink";
const sectionTitle =
    "flex items-center gap-2 text-xs font-bold tracking-wider text-stone-500 uppercase dark:text-stone-400";
const fieldCls =
        ...
        `}
            >
                ...
                <div className="flex items-center justify-between lg:hidden">
                    <span className="text-sm font-bold tracking-wide text-stone-700 uppercase dark:text-stone-300">
                        Menu
                    </span>
                    <button
                        type="button"
                ...
                <section className={`${card} flex min-h-0 flex-1 flex-col p-4 sm:p-5`}>
                    ...
                    <div className="max-h-64 min-h-0 flex-1 space-y-2 overflow-y-auto scrollbar-slim pr-1 lg:max-h-none">
                        {rooms.length === 0 ? (
                            ...
                            rooms.map((room) => (
                                <div
                                    key={room}
                                    className={`group flex w-full items-center justify-between gap-2 rounded-xl border px-3.5 py-3 text-left text-sm transition-all duration-200 ${
                                        joinedRoom === room
                                ...
                                >
                                    ...
                                    <span className="flex shrink-0 items-center gap-1.5">
                                        <span
                                            className={`text-[11px] font-bold ${joinedRoom === room ? "text-emerald-600 dark:text-emerald-400" : "text-stone-400 group-hover:text-emerald-600 dark:group-hover:text-emerald-400"}`}
                                        >
                    ...
                    <div className="mt-4 border-t border-stone-100 pt-4 dark:border-stone-800">
                        <div className="flex items-center justify-between text-xs">
                            ...
                            <span
                                className={`inline-flex items-center gap-1.5 font-bold ${joinedRoom ? "text-emerald-600 dark:text-emerald-400" : "text-stone-400 dark:text-stone-500"}`}
                            >
                                <span
                                    className={`h-1.5 w-1.5 rounded-full ${joinedRoom ? "animate-pulse bg-emerald-500" : "bg-stone-300 dark:bg-stone-600"}`}
                                />
